...
pub fn home() -> Html {
    ...
    // let album_data: yew::UseStateHandle<Vec<Album>> = use_state(||vec![]);
    // let album_data = vec![];
    ...
    // let onscroll = Callback::from(move|e: WheelEvent| {
        ...
        // if let Some(_) = e.target_dyn_into::<HtmlElement>() {
            ...
            // if scroll_height - (scroll_y + client_height) == 0.0 {
                // page.set(*page+1);
                // // wasm_bindgen_futures::spawn_local(async move {
                    // // // set_page_loading(true, dispatch.clone());
                    // // page.set(*page+1);
    ...
    <>
        ...
        if let Some(data) = items.data.clone() {
            {
                data.iter().map(move |album| {
